...
def test_generate_diff_nested():
    ...
    json_result_plain = generate_diff(json_file1, json_file2, 'plain')
    yaml_result_plain = generate_diff(yaml_file1, yaml_file2, 'plain')

    assert json_result == expected
    assert yaml_result == expected
    assert json_result_plain == expected_plain
    assert yaml_result_plain == expected_plain
